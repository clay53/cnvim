{
  ...
  outputs = { self, nixpkgs, nixvimFlake }:
    let
      ...
      importsBySystem = system: rec {
        pkgs = nixpkgs.legacyPackages.${system};
        nixvimLib = nixvimFlake.lib.${system};
        nixvim = nixvimFlake.legacyPackages.${system};
        nixvimModule = {
          inherit pkgs;
          module = {
            imports = [
    ...
    in
  {
    checks = forAllSystems (system: let imports = importsBySystem system; in with imports; {
      default = nixvim.makeNixvimWithModule nixvimModule;
    });
    packages = forAllSystems (system: let imports = importsBySystem system; in with imports; {
      default = nixvim.makeNixvimWithModule nixvimModule;
    });
  };
}
